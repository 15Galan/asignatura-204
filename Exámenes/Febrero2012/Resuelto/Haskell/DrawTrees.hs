...
_drawOnWithConf file tConf vConf showKV t = do
...
oval (P x y) w h = 
  ...
  where
   ...
   tY = y+ry
   bY = y-ry 
   lArc = arc (P lX y) ry ry (pi/2) (-pi/2)
...
renderNode node getSubtrees tConf vConf showKV dx dy = 
  Render g w h cx cy 
   where
    ...
    cx = if null centers then dx
         else if odd (length centers) then centers !! (length centers `div` 2) 
                                 else (head centers + last centers)/2
    cy = dy-rN
    ...
    g = sec [ With [Brush (shadowC vConf), Fill (shadowC vConf)] $ ov 2 (-2)
            ...
            , With [Brush (textC vConf), TextCentered True, Font (fontName vConf) (fontSz vConf) ] $ text (P cx cy) (showKV node)
