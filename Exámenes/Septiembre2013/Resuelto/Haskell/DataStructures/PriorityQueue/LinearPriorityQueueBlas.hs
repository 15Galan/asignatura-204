...
-- Pepe Gallardo, 2012
-- Blas Ruiz, 2012 -- cambio el comportamiento de enqueue 
-------------------------------------------------------------------------------

module DataStructures.PriorityQueue.LinearPriorityQueueBlas 
  ( PQueue
  , empty
  , isEmpty
  , first
...
enqueue x (Node y q) 
 ...
 -- -- nuevo comportamiento: Los elementos 
 -- -- con igual prioridad salen en orden FIFO   
 | y <= x        = Node y (enqueue x q)  -- dejamos primero el que llegó antes; 
...
-- This instace is used by QuickCheck to generate random Priority Queues
