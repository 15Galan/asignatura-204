...
module DataStructures.PrioritySearchQueue.PrioritySearchQueue
    ( 
    -- * Binding Type
    Binding((:->))
    , key
...
isEmpty :: PSQ k p -> Bool
isEmpty Void = True
isEmpty (Winner _ _ _ _) = False
...
minElem :: (Ord k, Ord p) => PSQ k p -> Binding k p
minElem Void             = error "minElem on empty priority search queue"
minElem (Winner k p t m) = k :-> p 

-- | /O(log n)/ Remove the binding with the lowest priority.
delMin :: (Ord k, Ord p) => PSQ k p -> PSQ k p
delMin Void             = Void
delMin (Winner k p t m) = secondBest t m
